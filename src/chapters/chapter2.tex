...
\begin{figure}[H]
    \centering
    \makebox[\textwidth]{\includegraphics[width=\linewidth]{src/assets/images/microservices.png}}
...
\subsection{Definition}
Testing is the process of executing a program to find errors. To make our software perform well it should be error-free. Also it makes refactoring and adding code a lot easier so it prevent the developer from breaking existing feature when adding new one.
\medskip
Microservices architectures make applications easier to scale and faster to develop, enabling innovation and accelerating time-to-market for new features.
\newline
There are multiple types of testing that we used in our application, here are their definitions below :
...
\begin{itemize}
    ...
    \item \textbf{Integration Testing:} After each unit is thoroughly tested, it is integrated with other units to create modules or components that are designed to perform specific tasks or activities. These are then tested as group through integration testing to ensure whole segments of an application behave as expected.
\end{itemize}
